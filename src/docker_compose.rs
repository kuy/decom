...
pub async fn containers() -> Result<Vec<String>, Box<dyn Error>> {
    let output = Command::new("docker-compose")
        ...
        .output()
        .await?;
    Ok(parse_ps_result(std::str::from_utf8(
        output.stdout.as_slice(),
    )?))
}

fn parse_ps_result(result: &str) -> Vec<String> {
    result.lines().into_iter().map(|s| s.to_string()).collect()
}
...
mod tests {
    ...
    fn test_parse_ps_result() {
        let text = "88c904a13f039e3eca4ff145cb06d8244548a341e56212f7e43598c2c4f62e8a\n\
        ...
        assert_eq!(parse_ps_result(text), expected);
    }
}
